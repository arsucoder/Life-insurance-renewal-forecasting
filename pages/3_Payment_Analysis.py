    ...
    <div class="hero">
        <div>
            <div class="hero-badge">
                <span class="hero-badge-dot"></span> Payment performance
            </div>
...
# FILTERS
# -----------------------------
# Date range gets more room than the payment-mode dropdown so the two
...
with col1:
    ...
    payment_modes = sorted(df["payment_mode"].dropna().unique())    selected_payment = st.selectbox("Select Payment Mode", payment_modes, label_visibility="collapsed")
...
# -----------------------------
# CHART ROW — Mode Comparison + Highlights
# -----------------------------
...
with chart5:
    st.markdown(
        '<div class="kcard"><div class="kcard-title">'
        f'<span class="dot" style="background:{BLUE};"></span>'
        'Renewal Rate by Payment Mode</div>',
        unsafe_allow_html=True,
    )
...
with st.expander("Detailed tables"):
    st.markdown(
        '<div class="section-label"><span class="bar"></span>Payment Mode Comparison</div>',
        unsafe_allow_html=True,
    )
