...
selected_policy_type = st.session_state["selected_policy_type"]
...
st.markdown(
    ...
    <div class="hero">
        <div>
            <div class="hero-badge">Policy performance</div>
            <div class="hero-title">Policy Type Analysis</div>
            <div class="hero-pills">
                <span class="hero-pill"><span class="pill-dot" style="background:#60A5FA;"></span>{selected_policy_type}</span>
...
with f1:
    ...
    chosen = st.selectbox(
        "Select Policy Type",
        policy_types,
        index=policy_types.index(selected_policy_type),
        label_visibility="collapsed",
        key="policy_type_select",
    )
    if chosen != st.session_state["selected_policy_type"]:
        st.session_state["selected_policy_type"] = chosen
        st.rerun()
...
with f3:
    st.markdown(
        f'<div class="filter-info"><b>{len(policy_types)}</b> policy types &middot; '
...
# ----- Row: Type Comparison + Highlights -----
c5, c6 = st.columns(2)

with c5:
    with st.container(border=True, key="chart_card_type_comparison"):
        st.markdown('<div class="chart-title">Renewal rate by policy type</div>', unsafe_allow_html=True)
        ...
        fig_comp = px.bar(comparison, x="policy_type", y="renewal_rate", color_discrete_sequence=[BLUE])
    ...
    with st.container(border=True, key="chart_card_highlights"):
        ...
        st.markdown(
            ...
            <div class="hl-row">
                ...
                <div class="hl-text"><b>Coverage</b> — {n_months} months of data for <b>{selected_policy_type}</b></div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
with st.expander("Detailed tables"):
    st.markdown(
        '<div class="section-label"><span class="bar" style="background:#2563EB;"></span>Policy Type Comparison</div>',
        unsafe_allow_html=True,
    )
...
st.markdown(
    ...
    <div class="explore-grid">
        <div class="explore-card">
            ...
            <div>
                <div class="explore-title">Previous: Payment Analysis</div>
                <div class="explore-sub">Renewal behaviour across payment modes.</div>
            </div>
        ...
        <div class="explore-card">
            ...
            <div>
                <div class="explore-title">Next: Region Analysis</div>
                <div class="explore-sub">Renewal behaviour across regions.</div>
            </div>
