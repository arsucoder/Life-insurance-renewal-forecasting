...
REGION_FILE = "data/processed/monthly_by_region.csv"
...
if not os.path.exists(REGION_FILE):
    ...
    st.info(
        "Make sure monthly_by_region.csv is inside "
        "data/processed/"
    )
...
if len(date_range) == 2:
    ...
    region_df = region_df[
        (region_df["collection_month"] >= start_date)
        &
        (region_df["collection_month"] <= end_date)
    ]
...
st.caption(
    f"{len(region_df)} monthly observations"
)
...
total_policies = region_df[
    "total_policies"
].sum()

renewed_policies = region_df[
    "renewed_policies"
].sum()

total_premium = region_df[
    "total_premium"
].sum()

renewed_premium = region_df[
    "renewed_premium"
].sum()
...
best_month = region_df.loc[best_idx]
worst_month = region_df.loc[worst_idx]
...
csv_data = region_df.to_csv(
    index=False
).encode("utf-8")


st.download_button(
    label="⬇️ Download Regional Data",
    data=csv_data,
